...
class CrossModalAttention(nn.Module):
    def __init__(self, modal_dim, out_dim=None, num_heads=2, dropout=0.15):
        super().__init__()
    ...
    def forward(self, ncrna_feat, drug_feat, disease_feat, ncrna_ids, drug_ids, disease_ids,
        ...
        details = {
            "batch_nc": batch_nc.squeeze(1),
            "batch_dg": batch_dg.squeeze(1),
            "batch_dis": batch_dis.squeeze(1),
            "gate_nc_dg": gate_dg.squeeze(1),
            "gate_nc_dis": gate_dis.squeeze(1),
            "gate_dg_dis": gate_dgdis.squeeze(1),
            "nc_dg_gated": nc_dg_gated.squeeze(1),
            "nc_dis_gated": nc_dis_gated.squeeze(1),
            "dg_dis_gated": dg_dis_gated.squeeze(1),
...
class NcrnaDrugDiseaseDualBranchModel(nn.Module):
    ...
    def forward(self, ncrna_feat, drug_feat, disease_feat, ncrna_ids, drug_ids, disease_ids,
                hyperedge_index=None, sign_weights=None, return_explain=False):
        # 步骤1：单模态原型对齐
